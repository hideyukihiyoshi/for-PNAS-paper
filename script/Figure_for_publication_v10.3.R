...
tmp.melt <- tmp.melt[c("brown", "green", "magenta", "red")]
tmp.melt$id <- rownames(tmp.melt); tmp.melt <- melt(tmp.melt, id.vars = "id")
ls.variable <- c("magenta", "red", "green", "brown")
ls.id <- unique(tmp.melt$id); ls.id <- sort(ls.id, decreasing = T)
png("Result/Figure/heatmap_vitro_RCA_filter_reference_coord_flip_v2.png", width = 2250, height = 900, res = 300)
ggplot(tmp.melt, aes(x = factor(variable, ls.variable), y = factor(id, ls.id), fill = value)) + geom_tile() + scale_fill_gradient(low = "white", high = "red") + geom_text(aes(label = round(value,2)), size = 2.5) + coord_flip() + theme_bw() + 
