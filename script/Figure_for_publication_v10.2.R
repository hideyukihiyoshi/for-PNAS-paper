...
png("Result/Figure/heatmap_vitro_RCA_filter_reference_coord_flip.png", width = 3000, height = 1200, res = 300)
ggplot(tmp.melt, aes(x = variable, y = id, fill = value)) + geom_tile() + scale_fill_gradient(low = "white", high = "red") + geom_text(aes(label = round(value,2)), size = 1.8) + coord_flip() + theme_bw() + 
